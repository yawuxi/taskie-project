...
const onTaskRemove = (columns: iProjectsColumnTypes[], id: string, uuid: string) => {
  //deep copy of columns data + gathering all task into one array
  ...
  //finding removable task from all tasks array
  const removableTask = allTasks.find(task => task.id === id)!;

  //iterating by columns and find column where is removable task
  copyOfColumns.map(column => {
    if (column.columnType === removableTask.columnType) {
      //finding removable task index and removing it
      ...
      <div className="projects-task__content">
        <header>
          ...
          <div className="projects-task__controls">
            {/*<button onClick={() => onTaskEdit(columns, id, uuid, dispatch)}>*/}
